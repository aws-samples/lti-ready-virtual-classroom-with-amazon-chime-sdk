...
function displayHelp() {
    ...
    console.log(`-p \t When running in pipeline mode, a pipeline will load the current parameters from AWS System Manager Parameter Store into cache.config.json`)
...
async function loadParametersSSM() {
    ...
    for (const param of configParams.parameters) {
        ...
        const loadedParam = await ssmClient.send(new GetParameterCommand({ Name: `${configParams.hierarchy}${param.name}` })).catch(error=>{
            ...
            throw `Could not find ${configParams.hierarchy}${param.name}, please make sure the backend ssm configuration has completed before running the frontend stack`;
        })
...
async function deleteParametersSSM() {
    ...
    for (const param of configParams.parameters) {
        ...
        await ssmClient.send(new DeleteParameterCommand({ Name: `${configParams.hierarchy}${param.name}` })).catch(error => console.log("Error deleting parameter ", error.message))
    }
...
async function run() {
    try {
        ...
        if(argFlags['p'] !== true) checkRequiredParameters()

        writeConfigCacheJSON()

        if (argFlags['t'] !== true && argFlags['p'] !== true) {
